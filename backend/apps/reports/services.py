...
class StockReportService:
    ...
    def get_general_stock(organization, filters: Dict = None) -> Dict[str, Any]:
        ...
        Retorna todos os itens com suas quantidades atuais.
        """
        ...
        items = ItemEstoque.objects.filter(
            ...
            ativo=True
        ).select_related().prefetch_related('lotes')
        ...
        for item in items:
            lotes = item.lotes.all()
            total_qty = sum(l.quantidade_atual or 0 for l in lotes)
            avg_cost = sum((l.quantidade_atual or 0) * (l.custo_unitario or 0) for l in lotes)
            ...
            if total_qty > 0:
                results.append({
                    ...
                    'quantity': total_qty,
                    'avg_cost': round(avg_cost / total_qty, 2) if total_qty > 0 else 0,
                    'total_value': round(avg_cost, 2),
                    'min_stock': float(item.estoque_minimo) if item.estoque_minimo else 0,
                    'is_low_stock': total_qty <= float(item.estoque_minimo) if item.estoque_minimo else False,
                })
                total_value += avg_cost
                total_items += 1
    ...
    def get_stock_movement(organization, date_range: Dict = None) -> Dict[str, Any]:
        ...
        ).select_related('item', 'lote', 'created_by')
        ...
        movements = queryset.order_by('-created_at')[:500]
        
        by_type = queryset.values('tipo_movimentacao').annotate(
            count=Count('id'),
        ...
        ).values('month', 'tipo_movimentacao').annotate(
        ...
        for m in movements:
            results.append({
                ...
                'type': m.get_tipo_movimentacao_display(),
                'type_code': m.tipo_movimentacao,
                'item_code': m.item.codigo if m.item else None,
                'item_name': m.item.nome if m.item else None,
                'quantity': float(m.quantidade) if m.quantidade else 0,
                'lote_code': m.lote.codigo_lote if m.lote else None,
                'created_by': m.created_by.full_name if m.created_by else None,
                'notes': m.observacoes or ''
            })
        ...
        return {
            ...
            'summary': {
                ...
                'by_type': [
                    {'type': item['tipo_movimentacao'], 'count': item['count'], 'quantity': float(item['total_quantity'] or 0)}
                    for item in by_type
                ],
                'monthly': [
                    {'month': str(item['month'].strftime('%Y-%m')) if item['month'] else None, 'type': item['tipo_movimentacao'], 'quantity': float(item['total'] or 0)}
    ...
    def get_low_stock(organization) -> Dict[str, Any]:
        ...
        for item in items:
            lotes = item.lotes.all()
            total_qty = sum(l.quantidade_atual or 0 for l in lotes)
            min_stock = float(item.estoque_minimo)
        ...
        results.sort(key=lambda x: (x['urgency'] == 'warning', -x['needed']))
    ...
    def get_expiry_report(organization, days_ahead: int = 30) -> Dict[str, Any]:
        """
        Relatório de validade de itens (medicamentos/vacinas).
        """
        ...
        ).select_related('item').order_by('data_validade')
        
        results = []
        total_value = 0
        
        for lote in lotes:
            value = (lote.quantidade or 0) * (lote.custo_unitario or 0)
            days_until = (lote.data_validade - date.today()).days
            ...
            results.append({
                ...
                'lote_code': lote.codigo_lote,
                'quantity': float(lote.quantidade) if lote.quantidade else 0,
                'expiry_date': lote.data_validade.isoformat(),
                'days_until_expiry': days_until,
                'cost': float(lote.custo_unitario) if lote.custo_unitario else 0,
                'total_value': round(value, 2),
                'urgency': 'critical' if days_until <= 7 else 'warning' if days_until <= 15 else 'normal'
            })
            total_value += value
        
        expired = LoteEstoque.objects.filter(
            item__organization=organization,
            item__categoria__in=['medicamento', 'vacina'],
        ...
        return {
            ...
            'total_expiring_value': round(total_value, 2),
            'total_expired_value': round(expired_value, 2)
        }
...
class FinancialReportService:
    ...
    def get_cashflow(organization, date_range: Dict = None) -> Dict[str, Any]:
        ...
        ).values('month', 'category__category_type').annotate(
            total=Sum('amount')
        ).order_by('month')
        ...
        return {
            ...
            'monthly': [
                {
                    'month': str(item['month'].strftime('%Y-%m')) if item['month'] else None,
    ...
    def get_dre(organization, date_range: Dict = None) -> Dict[str, Any]:
        ...
        # Revenue breakdown
        revenue_items = queryset.filter(
            category__category_type=FinancialCategory.CategoryType.REVENUE
        ).values('category__name').annotate(total=Sum('amount')).order_by('-total')
        
        # Expense breakdown
        expense_items = queryset.filter(
            category__category_type=FinancialCategory.CategoryType.EXPENSE
        ).values('category__name').annotate(total=Sum('amount')).order_by('-total')
        ...
        return {
            ...
            'revenue': {
                ...
                'items': [{'category': r['category__name'], 'amount': float(r['total'] or 0)} for r in revenue_items]
            },
            'expense': {
                'total': float(total_expense),
                'items': [{'category': r['category__name'], 'amount': float(r['total'] or 0)} for r in expense_items]
            },
            'result': {
                'gross_profit': float(total_revenue - total_expense),
                'margin': round((total_revenue - total_expense) / total_revenue * 100, 2) if total_revenue > 0 else 0
            },
            'monthly': [
                {'month': str(m['month'].strftime('%Y-%m')) if m['month'] else None, 'type': m['category__category_type'], 'amount': float(m['total'] or 0)}
    ...
    def get_by_category(organization, category_type: str = None) -> Dict[str, Any]:
        ...
        for cat in categories:
            transactions = queryset.filter(category__id=cat['category__id']).order_by('-created_at')[:20]
            results.append({
    ...
    def get_comparative(organization, year: int = None) -> Dict[str, Any]:
        ...
        return {
            'year': year,
            'monthly': [
                {'month': m, 'revenue': monthly_data[m]['revenue'], 'expense': monthly_data[m]['expense'], 'balance': monthly_data[m]['revenue'] - monthly_data[m]['expense']}
                for m in months
            ],
            'totals': {
...
class LivestockReportService:
    ...
    def get_inventory(organization) -> Dict[str, Any]:
        ...
        by_species = batches.values('species__name').annotate(
            total_animals=Sum('quantity'),
            batches=Count('id')
        ).order_by('-total_animals')
        
        by_farm = batches.values('farm__name').annotate(
            total_animals=Sum('quantity'),
            batches=Count('id')
        ).order_by('-total_animals')
        ...
        for batch in batches:
            results.append({
                ...
                'avg_weight': float(batch.avg_weight_kg) if batch.avg_weight_kg else None
            })
        
        return {
            'items': results,
            'by_species': [
                {'species': s['species__name'] or 'Não definido', 'total': s['total_animals'], 'batches': s['batches']}
                for s in by_species
            ],
            'by_farm': [
                {'farm': f['farm__name'], 'total': f['total_animals'], 'batches': f['batches']}
                for f in by_farm
            ],
            'by_status': [
                {'status': s['status'], 'total': s['total_animals'], 'batches': s['batches']}
                for s in by_status
            ],
...
class CropReportService:
    ...
    def get_area_planted(organization) -> Dict[str, Any]:
        ...
        by_farm = fields.values('farm__name').annotate(
        ...
        cycles_by_crop = active_cycles.values('crop_name').annotate(
            cycles=Count('id'),
        ...
        return {
            'summary': {
                ...
                'total_area': float(fields.aggregate(total=Sum('area_ha'))['total'] or 0),
                'active_cycles': active_cycles.count()
            },
            'by_farm': [
                {'farm': f['farm__name'], 'area': float(f['total_area'] or 0), 'fields': f['fields_count']}
                for f in by_farm
            ],
            'by_status': [
                {'status': 'Ativo' if s['is_active'] else 'Inativo', 'area': float(s['total_area'] or 0), 'fields': s['fields_count']}
                for s in by_status
            ],
            'by_crop': [
                {'crop': c['crop_name'] or 'Não definida', 'cycles': c['cycles'], 'area': float(c['total_area'] or 0)}
    ...
    def get_harvest(organization, year: int = None) -> Dict[str, Any]:
        ...
        by_crop = cycles.values('crop_name').annotate(
            cycles=Count('id'),
            total_area=Sum('field__area_ha'),
        ...
        by_farm = cycles.values('field__farm__name').annotate(
            cycles=Count('id'),
        ...
        for cycle in cycles:
            results.append({
                ...
                'expected_harvest': cycle.expected_harvest_date.isoformat() if cycle.expected_harvest_date else None,
                'expected_production': float(cycle.expected_production) if cycle.expected_production else 0,
                'actual_production': float(cycle.actual_production) if cycle.actual_production else 0,
                'status': cycle.get_status_display(),
                'productivity': round(float(cycle.actual_production or 0) / float(cycle.field.area_ha), 2) if cycle.actual_production and cycle.field.area_ha else 0
            })
        ...
        return {
            ...
            'by_crop': [
                {'crop': c['crop_name'] or 'Não definida', 'cycles': c['cycles'], 'area': float(c['total_area'] or 0), 'expected': float(c['total_expected'] or 0), 'harvested': float(c['total_harvested'] or 0)}
                for c in by_crop
            ],
            'by_farm': [
                {'farm': f['field__farm__name'], 'cycles': f['cycles'], 'production': float(f['total_production'] or 0)}
                for f in by_farm
            ],
            'cycles': results
        }