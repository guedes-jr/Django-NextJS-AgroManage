...
import { BadgePercent, Check, Pencil, Plus, Tags } from "lucide-react";

import { useToast } from "@/components/ui/Toast";
import { platformService } from "@/services/platformApi";
import type { PlatformPlan, PlatformSubscription, PlatformSubscriptionPage } from "@/types/platform";
...
export default function PlatformPlansPage() {
  ...
  const [discounting, setDiscounting] = useState<PlatformSubscription | null>(null);
  ...
  const openDiscount = (subscription: PlatformSubscription) => {
    setDiscounting(subscription);
    setDiscountForm({
      discount_type: subscription.discount_type || "percentage",
      discount_value: subscription.discount_value || "0.00",
      duration: subscription.discount_ends_at ? "temporary" : "permanent",
      discount_starts_at: subscription.discount_starts_at?.slice(0, 16) || "",
      discount_ends_at: subscription.discount_ends_at?.slice(0, 16) || "",
    });
  };
  const saveDiscount = async (event: FormEvent) => {
    ...
    setSaving(true);
    try {
      await platformService.setSubscriptionDiscount(discounting.id, {
        discount_type: Number(discountForm.discount_value) > 0 ? discountForm.discount_type : "",
        discount_value: discountForm.discount_value,
        discount_starts_at: discountForm.discount_starts_at ? new Date(discountForm.discount_starts_at).toISOString() : null,
        discount_ends_at: discountForm.duration === "temporary" && discountForm.discount_ends_at
          ? new Date(discountForm.discount_ends_at).toISOString() : null,
      });
      showToast("Desconto da assinatura atualizado.", "success");
      setDiscounting(null); load();
    } catch { showToast("Não foi possível aplicar o desconto.", "error"); }
    finally { setSaving(false); }
  };
  ...
  return <>
    ...
    {discounting && <div className="platform-card p-4 mb-4"><div className="d-flex justify-content-between mb-3"><div><h2 className="h5 fw-bold mb-1">Desconto — {discounting.organization_name}</h2><p className="text-muted small mb-0">Use valor zero para remover o desconto.</p></div><button className="btn-close" onClick={() => setDiscounting(null)} /></div><form onSubmit={saveDiscount}><div className="row g-3"><div className="col-md-3"><label className="form-label small fw-semibold">Tipo</label><select className="form-select" value={discountForm.discount_type} onChange={e=>setDiscountForm({...discountForm,discount_type:e.target.value as "percentage"|"fixed_amount"})}><option value="percentage">Percentual (%)</option><option value="fixed_amount">Valor em reais (R$)</option></select></div><div className="col-md-3"><label className="form-label small fw-semibold">Valor</label><input className="form-control" type="number" min="0" max={discountForm.discount_type === "percentage" ? "100" : undefined} step="0.01" value={discountForm.discount_value} onChange={e=>setDiscountForm({...discountForm,discount_value:e.target.value})} /></div><div className="col-md-3"><label className="form-label small fw-semibold">Duração</label><select className="form-select" value={discountForm.duration} onChange={e=>setDiscountForm({...discountForm,duration:e.target.value as "permanent"|"temporary"})}><option value="permanent">Permanente</option><option value="temporary">Temporário</option></select></div><div className="col-md-3"><label className="form-label small fw-semibold">Início (opcional)</label><input className="form-control" type="datetime-local" value={discountForm.discount_starts_at} onChange={e=>setDiscountForm({...discountForm,discount_starts_at:e.target.value})} /></div>{discountForm.duration === "temporary" && <div className="col-md-3"><label className="form-label small fw-semibold">Término</label><input className="form-control" required type="datetime-local" value={discountForm.discount_ends_at} onChange={e=>setDiscountForm({...discountForm,discount_ends_at:e.target.value})} /></div>}</div><button className="btn btn-success d-flex gap-2 align-items-center mt-3" disabled={saving}><Check size={16}/>{saving ? "Salvando..." : "Aplicar desconto"}</button></form></div>}
    <div className="platform-card overflow-hidden"><div className="p-4 border-bottom"><h2 className="h5 fw-bold mb-1">Assinaturas das organizações</h2><p className="text-muted small mb-0">{subscriptions?.count ?? 0} assinaturas cadastradas.</p></div><div className="table-responsive"><table className="table platform-table mb-0"><thead><tr><th>Organização</th><th>Plano</th><th>Ciclo</th><th>Desconto</th><th>Status</th><th>Gerenciar</th></tr></thead><tbody>{subscriptions?.results.map((subscription)=><tr key={subscription.id}><td><div className="fw-semibold">{subscription.organization_name}</div></td><td>{subscription.plan_name}</td><td>{subscription.billing_cycle_display}</td><td>{subscription.has_active_discount ? <span className="platform-status active">{subscription.discount_type === "percentage" ? `${Number(subscription.discount_value)}%` : `R$ ${Number(subscription.discount_value).toLocaleString("pt-BR",{minimumFractionDigits:2})}`}{subscription.discount_ends_at ? " temporário" : " permanente"}</span> : <span className="text-muted small">Sem desconto</span>}</td><td><span className={`platform-status ${subscription.status === "active" ? "active":"suspended"}`}>{subscription.status_display}</span></td><td><div className="d-flex gap-2"><select className="form-select form-select-sm" defaultValue={subscription.plan} id={`plan-${subscription.id}`}>{plans.filter(p=>p.is_active).map(plan=><option value={plan.id} key={plan.id}>{plan.name}</option>)}</select><button className="btn btn-outline-secondary btn-sm" onClick={()=>{const select=document.getElementById(`plan-${subscription.id}`) as HTMLSelectElement; void changePlan(subscription.id,select.value,subscription.billing_cycle);}}>Aplicar</button><button className="btn btn-outline-success btn-sm d-flex align-items-center gap-1" onClick={()=>openDiscount(subscription)}><BadgePercent size={14}/>Desconto</button></div></td></tr>)}</tbody></table></div></div>
  </>;
}
