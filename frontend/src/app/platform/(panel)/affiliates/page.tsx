...
export default function PlatformAffiliatesPage(){
  ...
  const [form,setForm]=useState({user_id:"",commission_type:"percentage",commission_value:"10.00",currency:"BRL"});
  ...
  const openCreate=()=>{setEditing(null);setForm({user_id:"",commission_type:"percentage",commission_value:"10.00",currency:"BRL"});setShowForm(true);};
  const openEdit=(affiliate:PlatformAffiliate)=>{setEditing(affiliate);setForm({user_id:affiliate.user_id,commission_type:affiliate.commission_type,commission_value:affiliate.commission_value,currency:affiliate.currency});setShowForm(true);};
  const save=async(event:FormEvent)=>{event.preventDefault();try{if(editing)await platformService.updateAffiliate(editing.id,form);else await platformService.createAffiliate(form);showToast(editing?"Comissão atualizada.":"Afiliado cadastrado.","success");setShowForm(false);load();}catch{showToast("Não foi possível salvar o afiliado.","error");}};
