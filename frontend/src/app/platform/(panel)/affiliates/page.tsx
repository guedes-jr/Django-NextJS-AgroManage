...
export default function PlatformAffiliatesPage(){
  ...
  const [form,setForm]=useState({user_id:"",full_name:"",email:"",initial_password:"",commission_type:"percentage",commission_value:"10.00",commission_duration:"first_payment",currency:"BRL"});
  ...
  const openCreate=()=>{setEditing(null);setAccountMode("dedicated");setForm({user_id:"",full_name:"",email:"",initial_password:"",commission_type:"percentage",commission_value:"10.00",commission_duration:"first_payment",currency:"BRL"});setShowForm(true);};
  const openEdit=(affiliate:PlatformAffiliate)=>{setEditing(affiliate);setAccountMode(affiliate.portal_access_only?"dedicated":"existing");setForm({user_id:affiliate.user_id,full_name:affiliate.full_name,email:affiliate.email,initial_password:"",commission_type:affiliate.commission_type,commission_value:affiliate.commission_value,commission_duration:affiliate.commission_duration,currency:affiliate.currency});setShowForm(true);};
  ...
  const save=async(event:FormEvent)=>{event.preventDefault();try{if(editing)await platformService.updateAffiliate(editing.id,{commission_type:form.commission_type,commission_value:form.commission_value,commission_duration:form.commission_duration,currency:form.currency});else if(accountMode==="dedicated")await platformService.createAffiliate({full_name:form.full_name,email:form.email,initial_password:form.initial_password,portal_access_only:true,commission_type:form.commission_type,commission_value:form.commission_value,commission_duration:form.commission_duration,currency:form.currency});else await platformService.createAffiliate({user_id:form.user_id,portal_access_only:false,commission_type:form.commission_type,commission_value:form.commission_value,commission_duration:form.commission_duration,currency:form.currency});showToast(editing?"Comissão atualizada.":"Afiliado cadastrado.","success");setShowForm(false);load();}catch{showToast("Não foi possível salvar o afiliado.","error");}};
  ...
  return <>
    ...
    {showForm&&<form className="platform-card p-4 mb-4" onSubmit={save}>
      <div className="d-flex justify-content-between mb-3"><h2 className="h5 fw-bold mb-0">{editing?"Editar comissão":"Cadastrar afiliado"}</h2><button type="button" className="btn-close" onClick={()=>setShowForm(false)}/></div>
      {!editing&&<div className="btn-group mb-3"><button type="button" className={`btn ${accountMode==="dedicated"?"btn-dark":"btn-outline-secondary"}`} onClick={()=>setAccountMode("dedicated")}>Acesso dedicado</button><button type="button" className={`btn ${accountMode==="existing"?"btn-dark":"btn-outline-secondary"}`} onClick={()=>setAccountMode("existing")}>Usuário existente</button></div>}
      <div className="row g-3">
        {!editing&&accountMode==="dedicated"?<><div className="col-md-4"><label className="form-label">Nome do vendedor</label><input className="form-control" required value={form.full_name} onChange={e=>setForm({...form,full_name:e.target.value})} onBlur={fillAffiliateEmail}/></div><div className="col-md-4"><label className="form-label">E-mail de acesso</label><input className="form-control" type="email" required value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/></div><div className="col-md-4"><label className="form-label">Senha inicial</label><input className="form-control" type="password" minLength={8} required value={form.initial_password} onChange={e=>setForm({...form,initial_password:e.target.value})}/></div></>:<div className="col-md-5"><label className="form-label">Usuário</label><select className="form-select" required disabled={!!editing} value={form.user_id} onChange={e=>setForm({...form,user_id:e.target.value})}><option value="">Selecione</option>{editing&&<option value={editing.user_id}>{editing.full_name} — {editing.email}</option>}{eligibleUsers.map(user=><option key={user.id} value={user.id}>{user.full_name} — {user.email}</option>)}</select></div>}
        <div className="col-md-3"><label className="form-label">Tipo</label><select className="form-select" value={form.commission_type} onChange={e=>setForm({...form,commission_type:e.target.value})}><option value="percentage">Percentual</option><option value="fixed_amount">Valor fixo</option></select></div>
        <div className="col-md-3"><label className="form-label">Duração da comissão</label><select className="form-select" value={form.commission_duration} onChange={e=>setForm({...form,commission_duration:e.target.value})}><option value="first_payment">Primeira mensalidade</option><option value="first_three_payments">3 primeiras mensalidades</option><option value="permanent">Permanente</option></select></div>
        <div className="col-md-2"><label className="form-label">Comissão</label><input className="form-control" type="number" min="0" max={form.commission_type==="percentage"?"100":undefined} step="0.01" required value={form.commission_value} onChange={e=>setForm({...form,commission_value:e.target.value})}/></div>
        <div className="col-md-2 d-flex align-items-end"><button className="btn btn-success w-100">Salvar</button></div>
      </div>
      {!editing&&accountMode==="dedicated"&&<div className="alert alert-light border mt-3 mb-0 small">A conta terá acesso somente pelo login principal e não será vinculada a uma organização.</div>}
    </form>}
