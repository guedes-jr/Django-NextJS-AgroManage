...
function ReproducaoPageContent() {
  ...
  const config: ReproducaoConfig = useMemo(() => ({
    ...
    flowSteps: [
      { id: "marras", icon: "🐖", label: "Marrãs", count: tab.marras?.rows?.length ?? 0, color: "#fff5f6", borderColor: "#fecdd3", accent: "#e84b67", image: "/images/reproduction/marras.png", summary: `${m.em_preparo ?? 0} em preparação` },
      { id: "matrizes", icon: "♀", label: "Matrizes", count: tab.matrizes?.rows?.length ?? 0, color: "#f2f7ff", borderColor: "#bfdbfe", accent: "#3978d4", image: "/images/reproduction/matrizes.png", summary: `${mat.cobertas ?? 0} coberturas pendentes` },
      { id: "gestacao", icon: "🤰", label: "Cobertura / Gestação", count: tab.gestacao?.rows?.length ?? 0, color: "#fff8ed", borderColor: "#fed7aa", accent: "#e98727", image: "/images/reproduction/gestacao.png", summary: `${g.aguardando_dg ?? 0} aguardando DG · ${g.confirmadas ?? 0} gestantes` },
      { id: "maternidade", icon: "🍼", label: "Maternidade", count: tab.maternidade?.rows?.length ?? 0, color: "#faf3ff", borderColor: "#e9d5ff", accent: "#9654d8", image: "/images/reproduction/maternidade.png", summary: `${matn.nascidos_mes ?? 0} nascidos neste mês` },
      { id: "creche", icon: "🐷", label: "Creche", count: tab.creche?.rows?.length ?? 0, color: "#f1fbf5", borderColor: "#bbf7d0", accent: "#249253", image: "/images/reproduction/marras.png", summary: `${cr.animais_alojados ?? 0} animais alojados` },
      { id: "crescimento", icon: "↗", label: "Crescimento", count: tab.crescimento?.rows?.length ?? 0, color: "#fffbef", borderColor: "#fde68a", accent: "#d59a16", image: "/images/reproduction/crescimento.png", summary: `${cresc.animais_alojados ?? 0} animais alojados` },
      { id: "engorda", icon: "⚖", label: "Engorda", count: tab.engorda?.rows?.length ?? 0, color: "#effafa", borderColor: "#99f6e4", accent: "#168f86", image: "/images/reproduction/crescimento.png", summary: `${eng.animais_alojados ?? 0} animais alojados` },
    ],
