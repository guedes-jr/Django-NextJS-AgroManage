  ...
  return <div className={styles.page}>
    ...
    <main className={styles.content}>
      ...
      <section className={styles.panel}><div className="d-flex justify-content-between align-items-center"><h2>2. Implementos vinculados</h2>{!selected && <span className="text-muted small">Salve ou selecione uma máquina primeiro.</span>}</div>
        {selected && <><form className="row g-2 align-items-end mb-3" onSubmit={addImplement}><div className="col-md-3"><label className="form-label">Implemento</label><input required className="form-control" value={implement.name} onChange={(e) => setImplement({ ...implement, name: e.target.value })} /></div><div className="col-md-3"><label className="form-label">Marca / Modelo</label><input className="form-control" value={implement.brand_model} onChange={(e) => setImplement({ ...implement, brand_model: e.target.value })} /></div><div className="col-md-1"><label className="form-label">Ano</label><input type="number" min="1900" max="2100" className="form-control" value={implement.manufacture_year || ""} onChange={(e) => setImplement({ ...implement, manufacture_year: e.target.value ? Number(e.target.value) : null })} /></div><div className="col-md-1"><label className="form-label">Qtd.</label><input type="number" min="1" className="form-control" value={implement.quantity} onChange={(e) => setImplement({ ...implement, quantity: Number(e.target.value) })} /></div><div className="col-md-2"><label className="form-label">Valor unitário</label><input type="number" min="0" step="0.01" className="form-control" value={implement.acquisition_value} onChange={(e) => setImplement({ ...implement, acquisition_value: e.target.value })} /></div><div className="col-md-2"><button className="btn btn-outline-success w-100"><Plus size={16} /> Adicionar</button></div></form>
        <div className="table-responsive"><table className="table table-sm align-middle"><thead><tr><th>Implemento</th><th>Marca / Modelo</th><th>Ano</th><th>Quantidade</th><th>Valor pago</th><th /></tr></thead><tbody>{selected.implements.map((item) => <tr key={item.id}><td>{item.name}</td><td>{item.brand_model || "—"}</td><td>{item.manufacture_year || "—"}</td><td>{item.quantity}</td><td>{money(Number(item.acquisition_value) * item.quantity)}</td><td className="text-end">{canDelete && <button className="btn btn-sm text-danger" onClick={() => void farmAssetService.removeImplement(item.id).then(refresh)}><Trash2 size={15} /></button>}</td></tr>)}</tbody><tfoot><tr className={styles.reportTotal}><td colSpan={4}>Total pago em implementos</td><td>{money(selected.implements_value)}</td><td /></tr></tfoot></table></div></>}
      </section>
