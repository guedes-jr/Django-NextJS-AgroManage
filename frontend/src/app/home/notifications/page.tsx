      ...
      : pageItems.map(notification => {
        ...
        return <article className={`${styles.item} ${!notification.is_read ? styles.unread : ""}`} key={notification.id}>
          <button className={styles.open} onClick={() => void openNotification(notification)} disabled={busyId === notification.id}>
            ...
            <span className={styles.content}><span className={styles.itemTop}><strong>{notification.title}{notification.occurrence_count > 1 && <b className="badge bg-success-subtle text-success ms-2">×{notification.occurrence_count}</b>}</strong><small>{notification.last_occurred_at || notification.created_at ? new Date(notification.last_occurred_at || notification.created_at).toLocaleString("pt-BR", { dateStyle: "medium", timeStyle: "short" }) : "Data não informada"}</small></span><span className={styles.message}>{notification.message}</span><span className={styles.meta}><i data-priority={notification.priority}/>{meta.label} · Prioridade {priorityLabel[notification.priority] || notification.priority}</span></span>
