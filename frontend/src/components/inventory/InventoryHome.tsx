...
export function InventoryHome() {
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.panel}><div className={styles.panelHead}><strong><WalletCards size={20}/> Resumo do Estoque</strong><span>Visão rápida do seu estoque para tomada de decisões.</span></div><div className={styles.stats}>
      ...
      <Link className={styles.stat} href="/home/estoque/produtos?status=baixo" aria-label={`Ver ${stats.estoque_baixo} itens com estoque baixo`} style={{ cursor: "pointer", color: "var(--foreground)", textDecoration: "none" }}><span className={styles.icon}><Warehouse size={35}/></span><div><small>Estoque baixo</small><strong>{stats.estoque_baixo}</strong><span>Itens abaixo do mínimo</span></div></Link>
    </div></section>
