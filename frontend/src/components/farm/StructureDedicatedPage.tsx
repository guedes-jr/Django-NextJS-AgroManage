...
import { ArrowLeft, ClipboardList, DollarSign, Pencil, Plus, Search, Trash2, Warehouse } from "lucide-react";
...
export function StructureDedicatedPage({
  ...
  useEffect(() => {
    ...
    Promise.all([farmStructureService.list(farmId), farmStructureService.summary(farmId)])
      .then(([listResponse, summaryResponse]) => { 
        if (active) { 
          const items = listResponse.data.results.filter((s: FarmStructure) => s.category === category);
          setStructures(items);
  ...
  const refresh = async () => {
    ...
    setStructures(items); 
    setSummary(summaryResponse.data);
    if (selected) setSelected(items.find((asset: FarmStructure) => asset.id === selected.id) || null);
  };
  ...
  const removeStructure = async (asset: FarmStructure) => {
    ...
    try { await farmStructureService.remove(asset.id); if (selected?.id === asset.id) resetForm(); await refresh(); showToast("Excluído com sucesso.", "success"); }
  ...
  return <div className={styles.page}>
    <header className={styles.hero}>
      ...
      <button className="btn btn-light fw-semibold" onClick={resetForm}><Plus size={17} /> Novo</button>
    </header>
    <main className={styles.content}>
      ...
      <section className={styles.panel}><h2>1. Dados: {categoryLabel}</h2><form onSubmit={saveStructure}><div className="row g-3">
        <Field label="Nome"><input required className="form-control" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder={`Ex.: ${defaultName}`} /></Field>
        ...
        /></div>
      </div><div className="d-flex justify-content-end gap-2 mt-4"><button type="button" className="btn btn-outline-secondary" onClick={resetForm}>Limpar</button><button className="btn btn-success px-4" disabled={saving || !farmId}>{saving ? "Salvando..." : (editing ? "Atualizar" : "Salvar")}</button></div></form></section>

      <section className={styles.panel}><div className="d-flex justify-content-between align-items-center"><h2>2. Materiais e itens utilizados</h2>{!selected && <span className="text-muted small">Salve ou selecione um item primeiro.</span>}</div>
        {selected && <><form className="row g-2 align-items-end mb-3" onSubmit={addMaterial}><div className="col-md-3"><label className="form-label">Item / Material</label><input required className="form-control" value={structureItem.name} onChange={(e) => setStructureItem({ ...structureItem, name: e.target.value })} list="dedicated-materials" /><datalist id="dedicated-materials">{defaultMaterials.map((opt) => <option key={opt} value={opt} />)}</datalist></div><div className="col-md-2"><label className="form-label">Unidade</label><select required className="form-select" value={structureItem.unit} onChange={(e) => setStructureItem({ ...structureItem, unit: e.target.value })}><option value="un">un</option><option value="m">m</option><option value="m²">m²</option><option value="m³">m³</option><option value="kg">kg</option><option value="L">L</option><option value="serviço">serviço</option></select></div><div className="col-md-2"><label className="form-label">Qtd.</label><input required type="number" min="0.01" step="0.01" className="form-control" value={structureItem.quantity} onChange={(e) => setStructureItem({ ...structureItem, quantity: e.target.value })} /></div><div className="col-md-3"><label className="form-label">Valor total</label><input type="number" min="0" step="0.01" className="form-control" required value={structureItem.value} onChange={(e) => setStructureItem({ ...structureItem, value: e.target.value })} /></div><div className="col-md-2"><button className="btn btn-outline-success w-100"><Plus size={16} /> Adicionar</button></div></form>
      ...
      <section className={styles.summary}><Summary icon={ClipboardList} label={`Total: ${categoryPlural}`} value={String(structures.length)} /><Summary icon={DollarSign} label="Valor investido" value={money(paid)} /><Summary icon={Warehouse} label="Valor atual (líquido)" value={money(current)} /></section>

      <section className={styles.panel}><div className="d-flex flex-wrap justify-content-between align-items-center gap-3"><div><h2 className="mb-1">3. Relatório completo</h2><p className="text-muted small mb-0">Relação consolidada e seus materiais da fazenda selecionada.</p></div><div className={styles.search}><Search size={17} /><input placeholder="Buscar estrutura ou material" value={search} onChange={(e) => setSearch(e.target.value)} /></div></div>
